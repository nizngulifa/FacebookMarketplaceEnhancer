...
| **`src/background/background.ts`** | **Service worker** — **`FME_BACKGROUND_SHOW_PROMPT`**, **`FME_BACKGROUND_SHOW_SUGGEST_REPLY`**, and **`FME_BACKGROUND_BRAIN_PREDICT`** ( **`fetch`** to local brain `POST /v1/predict` — no OpenAI key in the bundle ). |
| **`src/content/messenger.ts`** | Content script: **`FME_GET_THREAD_SNAPSHOT`**, **`FME_PROMPT_USER`**, **`FME_SUGGEST_REPLY`**, plus on each full Messenger load **`FME_BACKGROUND_SHOW_PROMPT`**, **`FME_BACKGROUND_BRAIN_PREDICT`**, **`FME_BACKGROUND_SHOW_SUGGEST_REPLY`** (tab from `sender.tab`) so writes use the **scripting** path. Popup “Reload messages” still uses `sendMessage` for snapshot; **responses to the caller can be `undefined`** on some Chrome + Messenger setups — prefer the **scripting** helpers from extension UI for writes that need a reliable return value. |
...
### E. Brain-powered suggestion (local server MVP)

The extension does **not** call OpenAI directly. Start the Python brain HTTP API from repo root:

```bash
make brain-serve
```

Then reload **messenger.com**. On load, the content script derives **`ChatInput`**-shaped turns and asks the worker to **`fetchSellerReply`** (`src/lib/brain-client.ts`) against **`http://127.0.0.1:8765`** by default (`src/lib/brain-config.ts`). **Secrets:** `OPENAI_API_KEY` only in `.env` next to the Python process ([docs/brain.md](../brain.md)).

Content script flow:

```ts
const pred = await chrome.runtime.sendMessage({
  type: "FME_BACKGROUND_BRAIN_PREDICT", // messenger-protocol.ts
  messages: [
    { role: "buyer", text: "…" },
    { role: "seller", text: "…" },
  ],
...
Map DOM rows to roles in **`thread-to-brain-messages.ts`** (e.g. **You** → seller in English). After `ok: true`, the same file usually chains **`FME_BACKGROUND_SHOW_SUGGEST_REPLY`** (see **`ghost-suggest-retry.ts`**).
...
| `FME_BACKGROUND_BRAIN_PREDICT` | content → service worker | `fetch` local brain `POST /v1/predict`; returns `{ ok, reply? }` (no API key in the extension). |

---

## 6. Manifest and bundles

- **`manifest.json`** — `permissions`, `host_permissions` (includes **messenger.com** + **http://127.0.0.1** / **http://localhost** for the local brain), `background.service_worker`, `content_scripts`, `action` popup.
...
- [ ] **`make brain-serve`** running; **`OPENAI_API_KEY`** set for Python.
- [ ] Full **reload** of a Messenger thread tab: confirm assistant **prompt** + brain **ghost suggest** (until gated for production).
...
- [docs/brain.md](../brain.md) — brain HTTP API, extension mapping (`You` → seller), troubleshooting.
